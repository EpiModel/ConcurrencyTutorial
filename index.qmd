...
The body of literature on concurrency is large, and our goal is not to summarize all of it (although we do provide links to some of it in the [More Resources](#moreresources) section). That literature has two major research paradigms within it: 
...
## EXERCISE 1: Thinking conceptually about concurrency at the local level {#exercise1}
...
In [Exercise 1](#exercise1), we set up the example so that Chris and Sam were similar on as many important metrics as possible, while still allowing Chris to have concurrent relationships, and Sam to have sequential relationships.  Recall the example:
...
#### Alternate Scenario 3. Sam and Chris begin and complete sex at the same time; Chris's relationships are twice as long as Sam's, but _Chris has only half as many coital acts with each partner as Sam does_.  Thus, overall Chris and Sam have the same total frequency of sex.
...
## EXERCISE 2: Thinking conceptually about concurrency at the population level {#exercise2}
...
## EXERCISE 3: Thinking numerically about concurrency at the local level {#exercise3} 
...
## EXERCISE 4: Thinking numerically about concurrency at the population level {#exercise4}
...
The R code for [Exercise 4](#exercise4) is printed below. Use the copy button at the top right of the code block to copy it, and paste it into a fresh .R file in your working directory (type `getwd()` to confirm this location). You can then run the code line by line, or run the entire simulation at once by typing
...
* [Network Modeling for Epidemics (NME) course materials](https://epimodel.github.io/sismid/)

# Tutorials 

## Tutorial 1: An introduction to the rules of probability {#tut1}
...
## Tutorial 2: An introduction to R {#tut2}
...
The webpage was authored by [Steven M. Goodreau](http://faculty.washington.edu/goodreau/), [Samuel M. Jenness](http://samueljenness.org/), and [Martina Morris](http://faculty.washington.edu/morrism).

All material is covered by a [GPL-3.0 license](https://github.com/EpiModel/ConcurrencyTutorial?tab=GPL-3.0-1-ov-file).

Code can be found in the associated [GitHub repository](https://github.com/EpiModel/ConcurrencyTutorial).
